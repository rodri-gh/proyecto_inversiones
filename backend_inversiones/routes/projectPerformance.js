...
import { ProjectPerformance } from '../models/mainExport.js';
...
router.get('/', async function (req, res, next) {
  try {
    const projects = await ProjectPerformance.findAll();
...
router.get('/:id', function (req, res, next) {
  const { id } = req.params;
  try {
    const project = ProjectPerformance.findOne({ where: { id } });
...
router.post('/', async (req, res, next) => {
  ...
  try {
    await ProjectPerformance.create({ name, description, investmentGoal, status,
...
router.put('/:id', async (req, res, next) => {
  ...
  try {
    const [projectCount] = await ProjectPerformance.update({ name, description, investmentGoal, status,
      startDate, endDate, projectType, profitPercentage }, {
      where: { id }
    });
...
router.patch('/:id', async (req, res, next) => {
  const { id } = req.params;
  try {
    const [projectDeleted] = await ProjectPerformance.update({ deleted: true }, {
      where: { id }
    });
...
router.put('/status/:id', async (req, res, next) => {
  ...
  try {
    ...
    const [projectCount] = await ProjectPerformance.update({ status: newStatus }, {
      where: { id }
    });
    verifyIfIdExists(projectCount);
    getHandleSuccess(204)(res);
