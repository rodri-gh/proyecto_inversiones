...
import { FinancialProjections } from '../models/mainExport.js';
...
router.get('/', async function (req, res, next) {
  try {
    const projects = await FinancialProjections.findAll();
...
router.get('/:id', function (req, res, next) {
  const { id } = req.params;
  try {
    const project = FinancialProjections.findOne({ where: { id } });
...
router.post('/', async (req, res, next) => {
  ...
  try {
    await FinancialProjections.create({ name, description, investmentGoal, status,
...
router.put('/:id', async (req, res, next) => {
  ...
  try {
    const [projectCount] = await FinancialProjections.update({ name, description, investmentGoal, status,
      startDate, endDate, projectType, profitPercentage }, {
      where: { id }
    });
...
router.patch('/:id', async (req, res, next) => {
  const { id } = req.params;
  try {
    const [projectDeleted] = await FinancialProjections.update({ deleted: true }, {
      where: { id }
    });
...
router.put('/status/:id', async (req, res, next) => {
  ...
  try {
    ...
    const [projectCount] = await FinancialProjections.update({ status: newStatus }, {
      where: { id }
    });
    verifyIfIdExists(projectCount);
    getHandleSuccess(204)(res);
