...
import { FinancialSettings } from '../models/mainExport.js';
...
router.get('/', async function (req, res, next) {
  try {
    const projects = await FinancialSettings.findAll();
...
router.post('/', async (req, res, next) => {
  ...
  try {
    await FinancialSettings.create({ name, description, investmentGoal, status,
...
router.put('/:id', async (req, res, next) => {
  ...
  try {
    const [projectCount] = await FinancialSettings.update({ name, description, investmentGoal, status,
      startDate, endDate, projectType, profitPercentage }, {
      where: { id }
    });
...
router.patch('/:id', async (req, res, next) => {
  const { id } = req.params;
  try {
    const [projectDeleted] = await FinancialSettings.update({ deleted: true }, {
      where: { id }
    });
...
router.put('/status/:id', async (req, res, next) => {
  const { id } = req.params;
  try {
    const project = FinancialSettings.findOne({ where: { id } });
    verifyIfIdExists(project);
    let newStatus;
    ...
    const [projectCount] = await FinancialSettings.update({ status: newStatus }, {
      where: { id }
    });
    verifyIfIdExists(projectCount);
    getHandleSuccess(204)(res);
