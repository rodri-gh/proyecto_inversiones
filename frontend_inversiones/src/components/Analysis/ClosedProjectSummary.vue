...
import { Chart as ChartJS, Title, Tooltip, BarElement, ArcElement, CategoryScale, LinearScale } from 'chart.js'
import ProjectMineral from '../ProjectMineral.vue';
import Investments from '../Investments.vue';
import { Bar } from 'vue-chartjs';

ChartJS.register(Title, Tooltip, ArcElement, BarElement, CategoryScale, LinearScale)
...
const addDetailsProjectObject =  () => { 
    ...
    if (Array.isArray(project.value.operatingExpenses)) {
        const totalExpenses = project.value.operatingExpenses.reduce((acc, item) => {
    ...
    if (Array.isArray(project.value.investments)) {
        const earnings = project.value.investments
            .map(investment => parseFloat(investment.earnings))
    ...
    cumplimientoMetaData.value = {
        ...
        datasets: [
            {
            data: [project.value.reuturnInvestment, parseFloat(project.value.investmentGoal)],
            backgroundColor: ['#4CAF50', '#F44336'],
            hoverOffset: 4
            }
        ],
            ...
            <div class="col-12 col-md-6 col-lg-5 mb-4">
                <div class="detail-item shadow">
                    ...
                    <ul>
                        ...
                        <li>
                        <strong>Ganancia total del Projecto:</strong>
                        {{ project.netProfit }}
                        </li>
